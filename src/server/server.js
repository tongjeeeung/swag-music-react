...
app.use(
  "/static",
  express.static(path.join(__dirname, "/swag-music-react/static")),
);
...
app.post("/user/register", async (req, res) => {
  ...
  try {
    ...
    // Генерация токенов
    const accessToken = jwt.sign({ id: uuidv4() }, "your_jwt_secret_key", {
      expiresIn: "1h",
    });
    ...
    if (error.name === "ValidationError") {
      res
        .status(400)
        .json({ message: "Ошибка валидации", error: error.errors });
    } else {
      res
        .status(500)
...
app.post("/user/login", async (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign({ id: user._id }, "your_jwt_secret_key", {
      expiresIn: "1h",
    });
    const refreshToken = jwt.sign(
      { id: user._id },
...
// Эндпоинт для получения массива лайкнутых треков, и плейлиста
app.get("/user/liked-tracks", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Ошибка при получении лайкнутых треков", error });
  }
});
...
app.post("/user/toggle-like-track", async (req, res) => {
  ...
  try {
    ...
    // Поиск трека среди всех плейлистов артистов
    const artists = await Artist.find({}, "playlists"); // Получаем только поле playlists у всех артистов
    const playlists = artists.reduce(
      (acc, artist) => acc.concat(artist.playlists),
      [],
    ...
    for (const playlist of playlists) {
      const track = playlist.tracks.find((t) => t._id.toString() === trackId);
      if (track) {
        foundTrack = track;
        break;
      }
    ...
    if (trackIndex > -1) {
      ...
      user.likedPlaylist.tracks.splice(trackIndex, 1);
    } else {
      // Трек не добавлен, добавляем его
      user.likedTracks.push(trackId);
      user.likedPlaylist.tracks.push(foundTrack);
    }
    ...
    res.json({
      message:
        trackIndex > -1
          ? `Трек удален из лайкнутых`
          : `Трек добавлен в лайкнутые`,
      likedTracks: user.likedTracks,
      likedPlaylist: user.likedPlaylist,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Ошибка при изменении лайкнутого трека", error });
  }
});
...
app.get("/user/playlists", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Ошибка при получении плейлистов пользователя", error });
  }
});
...
app.post("/user/toggle-added-playlist", async (req, res) => {
  ...
  try {
    ...
    if (playlistIndex > -1) {
      ...
      await user.save();
      return res.json({
        message: "Плейлист удален из добавленных",
        addedPlaylists: user.addedPlaylists,
      });
    } else {
      ...
      await user.save();
      return res.json({
        message: "Плейлист добавлен в добавленные",
        addedPlaylists: user.addedPlaylists,
      });
    }
  } catch (error) {
    res
      .status(500)
      .json({ message: "Ошибка при изменении добавленного плейлиста", error });
  }
});
...
app.put("/user/change-password", async (req, res) => {
  ...
  if (!email || !newPassword) {
    return res
      .status(400)
      .json({ message: "Почта и новый пароль должны быть предоставлены" });
  }
...
app.delete("/users", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Ошибка при удалении пользователей", error });
  }
});
...
app.get("/playlists", async (req, res) => {
  try {
    const artists = await Artist.find({}, "playlists"); // Получаем только поле playlists у всех артистов
    const artistPlaylists = artists.reduce(
      (acc, artist) => acc.concat(artist.playlists),
      [],
    );
    ...
    const userPlaylistsIds = users.reduce(
      (acc, user) => acc.concat(user.addedPlaylists),
      [],
    );
...
app.post("/playlists/create", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const playlistImage =
      typeof req.body.image === "string" ? req.body.image : uploadedImage;
    ...
    res
      .status(201)
      .json({ message: "Playlist saved successfully", playlist: newPlaylist });
  } catch (error) {
  ...
  try {
    ...
    if (!user.addedPlaylists.includes(id)) {
      return res
        .status(403)
        .json({
          error: "User does not have permission to update this playlist",
        });
    }
    ...
    updatedPlaylist.image = req.file
  ...
  try {
    ...
    if (!user.addedPlaylists.includes(playlistId)) {
      return res
        .status(403)
        .json({
          error: "User does not have permission to delete this playlist",
        });
    }
    ...
    user.addedPlaylists = user.addedPlaylists.filter(
...
app.get("/playlists/:id", async (req, res) => {
  ...
  try {
    if (playlistId === "likedSongs") {
      ...
      if (!user) {
        return res.status(404).json({ message: "Польzzzователь не найден" });
      }

      return res.json(user.likedPlaylist);
    }
...
app.post("/playlists/by-ids", async (req, res) => {
  ...
  if (!ids || !Array.isArray(ids)) {
    return res
      .status(400)
      .json({
        message: "Массив идентификаторов не указан или имеет неверный формат",
      });
  }
  ...
  try {
    ...
    const artists = await Artist.find(
      { "playlists._id": { $in: ids } },
      "playlists",
    );
    const artistPlaylists = artists.reduce((acc, artist) => {
      const filteredPlaylists = artist.playlists.filter((playlist) =>
        ids.includes(playlist._id),
      );
...
app.get("/search-track", async (req, res) => {
  ...
  if (!trackId) {
    return res
      .status(400)
      .json({ message: "Необходимо указать ID трека для поиска" });
  }
...
app.get("/popular", async (req, res) => {
  try {
    ...
    const artists = await Artist.find({}, "playlists").populate(
      "playlists.tracks",
    );
    ...
    for (const trackId of topTrackIds) {
      for (const playlist of playlists) {
        if (
          playlist.tracks.some((track) => track._id.toString() === trackId) &&
          !addedPlaylists.has(playlist._id.toString())
        ) {
    ...
    while (popularPlaylists.length < 3) {
      popularPlaylists.push(
        ...popularPlaylists.slice(0, 3 - popularPlaylists.length),
      );
    }
    ...
    res
      .status(500)
      .json({
        message: "Ошибка при получении популярных треков и плейлистов",
        error,
      });
  }
});
