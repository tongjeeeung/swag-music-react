...
export const Controler: FC = () => {
  ...
  const [currentAlbum, setCurrentAlbum] = useState<TPlaylist>(
    controler.currentPlaylist,
  );
  ...
  function audioTime(duration: number) {
    ...
    const seconds =
      Math.floor(duration - minutes * 60) < 10
        ? `0${Math.floor(duration - minutes * 60)}`
  ...
  useEffect(() => {
    ...
    if (user) {
      const like = user.likedTracks.find((id) => id === current._id)
        ? true
        : false;
  ...
  useEffect(() => {
    if (audio && controler.playing) {
      setTimeout(() => {
        setTime(audioTime(audio.currentTime));
      }, 1000);
    }
  }, [audio?.currentTime]);
  ...
  const prevHandle = () => {
    const prev =
      currentAlbum?.tracks[
        currentAlbum?.tracks.findIndex((track) => track._id === current._id) - 1
      ];
  ...
  const nextHandle = () => {
    if (controler.shuffle) {
      ...
      const next =
        currentAlbum?.tracks[
          currentAlbum?.tracks.findIndex((track) => track._id === current._id) +
            1
        ];
  ...
  const progressHandle = (evt: number) => {
    const progressWidth = window
      .getComputedStyle(document.querySelector("#range")!)
      .width.replace(/[a-z%]/gi, "");
  ...
  return (
    <ControlerUI
      ...
      isPlaying={controler.playing}
      isRepeat={controler.repeating}
      isShuffle={controler.shuffle}
      progressVoid={progressHandle}
      volumeVoid={volumeHandle}
      volumeButtonVoid={volumeMuteHandle}
      volumeWidth={volume}
      likeVoid={likeHandle}
      nextVoid={nextHandle}
      playVoid={playHandle}
      prevVoid={prevHandle}
      repeatVoid={repeatHandle}
      shuffleVoid={shuffleHandle}
    ></ControlerUI>
  );
};
