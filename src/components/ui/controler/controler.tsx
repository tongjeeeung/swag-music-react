...
export const ControlerUI: FC<TControler & { open: boolean }> = ({
  likeVoid,
  image,
  name,
  executorID,
  ...
  let volumeSvg2 = (
    <>
      (
      <path
        d="M1,22v3c0,0.343.007,0.829,0,1a0.556,0.556,0,0,0,0,.057A1.377,1.377,0,0,0,2,27H5l2,2,1,1H9V18H8L5,21a25.547,25.547,0,0,1-3,0,0.7,0.7,0,0,0-.227,0A1.318,1.318,0,0,0,1,22Z"
        transform="translate(4 -11)"
      />
      <path
        d="M10.226,21a0.752,0.752,0,0,1,0-1,1.09,1.09,0,0,1,.966.3c1.757,1.381.469,5.492-.1,6.807a1.076,1.076,0,0,1-.553.754A1.063,1.063,0,0,1,10.226,27c-0.077-1.227.838-1.939,1-3a3.621,3.621,0,0,0-.963-2.967C10.255,21.027,10.239,21.013,10.226,21Z"
        transform="translate(4 -11)"
      />
      )
    </>
  ...
  const timeWidth =
    ((Number(trackTimer.split(":")[0]) * 60 +
      Number(trackTimer.split(":")[1])) *
      100) /
  ...
  if (volumeWidth >= 0.5) {
    volumeSvg2 = (
      <>
        ...
        />
        <path
          d="M13.3,19.537a1.088,1.088,0,0,1,0-1.446c0.409-.273,1.179.269,1.4,0.441,2.542,2,.678,7.943-0.145,9.844a1.556,1.556,0,0,1-.8,1.09,1.538,1.538,0,0,1-.453-1.251c-0.112-1.775,1.213-2.8,1.446-4.339a5.236,5.236,0,0,0-1.392-4.291C13.345,19.575,13.321,19.555,13.3,19.537Z"
          transform="translate(4 -11)"
        />
        )
      </>
  ...
  const refName: null | { current: HTMLHeadingElement | null } = useRef(null);
  const refInfoBox: null | { current: HTMLDivElement | null } = useRef(null);

  useEffect(() => {
    if (
      refName.current &&
      refInfoBox.current &&
      refName.current.scrollWidth > refInfoBox.current.scrollWidth * 0.2
    ) {
      refName.current.style.animation = "scrollLeft 22s linear infinite";
    } else if (refName.current !== null) {
      refName.current.style.animation = "none";
    }
    ...
    if (refName.current) {
      ...
      @keyframes scrollLeft {
        0% { transform: translateX(${refName.current.scrollWidth + 1}px); }
        50% { transform: translateX(0); }
        100% { transform: translateX(-${refName.current.scrollWidth + 10}px); }
      }
    `,
  ...
  if (controler.current !== null && open) {
    const containHide = () =>
      controler.current!.classList.contains(`${styles.open}`);
  ...
  return (
    <div
      className={styles.controler + clsx(open ? ` ${styles.open}` : "")}
      ref={controler}
    >
      <div className={styles.current}>
        <img className={styles.image} src={image} />
        <div className={styles.info} ref={refInfoBox}>
          <h4 className={styles.title} ref={refName}>
            {name}
          </h4>
          <Link
      ...
      <div className={styles.menu}>
        <button className={styles.menu_button} onClick={shuffleVoid}>
          <svg
            className={clsx(styles.svg, { [styles.active]: isShuffle })}
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
        ...
        <button className={styles.menu_button} onClick={prevVoid}>
          <svg
            className={clsx(styles.svg + " " + styles.prevButton)}
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="currentColor"
        ...
        <button className={styles.menu_button} onClick={repeatVoid}>
          <svg
            className={clsx(styles.svg, { [styles.active]: isRepeat })}
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
      ...
      <div className={styles.volume_menu}>
        ...
        <div className={styles.volume_range}>
          <div
            style={{ width: `${volumeWidth * 100}px` }}
            className={styles.value}
          ></div>
          <div
            className={styles.input}
            id="volume"
            onClick={(evt) => volumeVoid(evt.nativeEvent.layerX / 100)}
          ></div>
        </div>
      </div>
      <div className={styles.controler_range}>
        ...
        <div className={styles.progress}>
          <div
            ...
            className={styles.full_range}
          ></div>
          <div
            className={styles.range_input}
            onClick={(evt) => progressVoid(evt.nativeEvent.layerX)}
            id="range"
          ></div>
        </div>
