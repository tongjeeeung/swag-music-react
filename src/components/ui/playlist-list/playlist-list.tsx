...
export const PlayListListUI: FC<TPlayListList> = ({ title, children, locationState, isAuth }) => (
  <section className={styles.playlists}>
    ...
    {isAuth ? (<Link to={'/swag-music-react/playlists/create/new-playlist'} state={locationState} className={styles.plus}>+</Link>) : ('')}
    <ul className={styles.playlist_list}>
      {children.length > 0 ? (children.map((item) => (
      <Link to={`/swag-music-react/playlists/${item._id}`} key={item._id} state={locationState} style={{ textDecoration: 'none', color: "inherit" }}>
