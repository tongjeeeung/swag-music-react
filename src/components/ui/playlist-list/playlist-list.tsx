...
export const PlayListListUI: FC<TPlayListList> = ({
  title,
  children,
  locationState,
  isAuth,
  ...
  <section className={styles.playlists}>
    ...
    {isAuth ? (
      <Link
        to={"/swag-music-react/playlists/create/new-playlist"}
        state={locationState}
    ...
    <ul className={styles.playlist_list}>
      {children.length > 0 ? (
        children.map((item) => (
          <PlaylistItemUI
            name={item.name}
            executor={item.executor !== "none" ? item.executor : ""}
            image={typeof item.image === "string" ? item.image : ""}
            key={item._id}
            id={item._id}
            executorId={item.executorID ? item.executorID : ""}
          ></PlaylistItemUI>
        ))
      ) : (
