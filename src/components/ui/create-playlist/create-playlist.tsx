...
export const CreatePlaylistUI: FC<CreateIUProps> = ({
  ...
  const handleDeleteButton = (track: TTrack) => {
    setSelectedTracks(
      selectedTracks.filter((_track) => _track._id !== track._id),
    );
  };
    ...
    <form className={styles.create} onSubmit={(e) => handleFormSubmit(e)}>
      <div className={styles.header}>
        ...
        <div className={styles.form}>
          ...
          />
          {localErrors.name && (
            <span className={styles.error}>{localErrors.name}</span>
          )}
          <textarea
          ...
          />
          {localErrors.info && (
            <span className={styles.error}>{localErrors.info}</span>
          )}
        </div>
      </div>
      ...
      <ul className={styles.addedTracks}>
        {selectedTracks.length == 0 ? (
          <h3 className={styles.nothing}>Sorry u don't add tracks</h3>
        ) : (
          selectedTracks.map((track) => (
            <li key={track._id} className={styles.track}>
              ...
              <button
                ...
                onClick={() => handleDeleteButton(track)}
              >
                X
              </button>
            </li>
      ...
      <div className={styles.buttons}>
        ...
        {id ? (
          <button className={styles.cancel} onClick={handleDeletePlaylist}>
            Delete
          </button>
        ) : (
          ""
        ...
        <button
          ...
          type="submit"
          disabled={selectedTracks.length === 0 || !image}
        >
          Create
        </button>
      </div>
