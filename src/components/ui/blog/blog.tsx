    ...
    <div className={styles.blog}>
      <ul className={styles.new}>
        <li className={styles.new_item}>
          <Link
            to={`/swag-music-react/blog/${children[children.length - 1]._id}`}
            key={children[children.length - 1]._id}
            style={{ textDecoration: "none", color: "inherit" }}
          >
            <div className={styles.box}>
              <img
                className={styles.image}
                src={children[children.length - 1].image}
                alt={children[children.length - 1].name}
              />
            </div>
            ...
            <h3 className={styles.title}>
              {children[children.length - 1].title}
            </h3>
        ...
        <li className={styles.new_item}>
          <Link
            to={`/swag-music-react/blog/${children[children.length - 2]._id}`}
            key={children[children.length - 2]._id}
            style={{ textDecoration: "none", color: "inherit" }}
          >
            <div className={styles.box}>
              <img
                className={styles.image}
                src={children[children.length - 2].image}
                alt={children[children.length - 2].name}
              />
            </div>
            <h2 className={styles.name}>
              {children[children.length - 2].name}
            </h2>
            <h3 className={styles.title}>
              {children[children.length - 2].title}
            </h3>
            <button className={styles.button}>Article</button>
          </Link>
        </li>
      ...
      <div className={styles.slider_box}>
        <ul
          ...
          style={{ transform: `translate(-${sliderValue}px, 0px)` }}
        >
          {children.map((article) => {
            return (
              <Link
                className={styles.slider_item}
                to={`/swag-music-react/blog/${article._id}`}
                key={article._id}
                style={{ textDecoration: "none", color: "inherit" }}
              >
      ...
      <ul className={styles.favorites}>
        <Link
          ...
          to={`/swag-music-react/blog/${children[children.length - 3]._id}`}
          key={children[children.length - 3]._id}
          style={{ textDecoration: "none", color: "inherit" }}
        >
          <img
            className={styles.image}
            src={children[children.length - 3].image}
            alt={children[children.length - 3].name}
          />
          <h2 className={styles.name}>{children[children.length - 3].name}</h2>
        </Link>
        <Link
          className={styles.favorites_item}
          to={`/swag-music-react/blog/${children[children.length - 4]._id}`}
          key={children[children.length - 4]._id}
          style={{ textDecoration: "none", color: "inherit" }}
        >
          <img
            className={styles.image}
            src={children[children.length - 4].image}
            alt={children[children.length - 4].name}
          />
      ...
      <ul className={styles.rand}>
        {children.map((_article, index) => {
          if (index < 3 && randArr.length > 0) {
            return (
              <Link
                to={`/swag-music-react/blog/${children[randArr[index]]._id}`}
                key={children[randArr[index]]._id}
                style={{ textDecoration: "none", color: "inherit" }}
              >
                <li className={styles.rand_item}>
                  <div className={styles.box}>
                    <img
                      className={styles.image}
                      src={children[randArr[index]].image}
                      alt={children[randArr[index]].name}
                    />
                  </div>
                  <h2 className={styles.name}>
                    {children[randArr[index]].name}
                  </h2>
                  <button className={styles.button}>Article</button>
                </li>
              </Link>
      ...
      <ul className={styles.last} ref={parent}>
        {children.map((article, index) => {
          if (index <= moreIndex) {
            return (
              <Link
